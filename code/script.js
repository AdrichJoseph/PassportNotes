const continent = [
  ...
  { name: "antarctica", color: "#20B2AA" }, // Corrected
  { name: "all", color: "#B0C4DE" },
];

// Selecting DOM elements
const btn = document.querySelector(".btn-open");
const form = document.querySelector(".fact-form");
const factsList = document.querySelector(".facts"); // Updated to match container
...
async function loadFacts() {
  try {
    const res = await fetch(
      "https://hxlnegsgozqzmjilieao.supabase.co/rest/v1/facts?",
      {
        headers: {
          apikey:
          ...
          authorization:
            "Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Imh4bG5lZ3Nnb3pxem1qaWxpZWFvIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTU1MTY1MzMsImV4cCI6MjA3MTA5MjUzM30.mnIWOtQsrvnYr3q3hQofUIGXDjmVNiQ_imnRmOQ6ClU",
        },
      }
    );
...
const htmlArr = dataArray.map((fact) => {
  console.log("Fact object:", fact); // Log each fact to see fields like 'category' or 'continent'
  ...
  return `<li class="fact">
    ...
    <span class="tag" style="background-color: ${cat.color
